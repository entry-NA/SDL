...
class BaseRunner(ABC):
    ...
    def load_checkpoint(self, checkpoint_path: Optional[str] = None) -> None:
        ...
        try:
            # accelerate saves to a directory containing model.safetensors
...
class StandardRunner(BaseRunner):
    ...
    def _build_dataloader(self) -> None:
        ...
        try:
            ...
            # Skip train dataloader in eval mode
            if getattr(self.config, 'mode', 'train') != 'eval':
    ...
    def _prepare_accelerator(self) -> None:
        ...
        # In eval mode, train_dataloader is intentionally None
        if getattr(self.config, 'mode', 'train') != 'eval':
            required_components['train_dataloader'] = self.train_dataloader
        ...
        try:
            # Prepare components for distributed training (skip train_dataloader in eval)
            if getattr(self.config, 'mode', 'train') != 'eval':
                (
                    self.model,
                    self.discriminator,
                ...
                self.model, self.discriminator = self.accelerator.prepare(

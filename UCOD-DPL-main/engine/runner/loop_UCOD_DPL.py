...
class TrainLoop(BaseLoop):
    ...
    def _process_batch(self, batch_data: Dict[str, torch.Tensor]) -> torch.Tensor:
        ...
        # Guard: only upsample pseudo_labels if not already at target size
        # (refined PNG labels from base_dataset are pre-resized to feature_size)
...
class ValLoop_Look_Twice(BaseLoop):
    ...
    def run(self):        
        ...
        for batch in tqdm(self.runner.val_dataloader):
            ...
            # normalize label_tensor to tensor
            if isinstance(label_tensor, (list, tuple)):
                label_tensor = torch.stack(label_tensor, dim=0) if isinstance(label_tensor[0], torch.Tensor) else torch.tensor(label_tensor)
            h = w = self.cfg.model_cfg.feature_size
            features = F.interpolate(features, size=(h,w), mode='bilinear')

...
def resolve_refined_pseudo_label_dir(
    cache_dir: str,
    refined_pseudo_label_dir: Optional[str],
) -> str:
    if refined_pseudo_label_dir is not None:
        return refined_pseudo_label_dir
    return os.path.join(cache_dir, 'refined_pseudo_labels')
...
class BaseCODDataset(Dataset):
    ...
    def __init__(
        ...
        self.refined_pseudo_label_dir = resolve_refined_pseudo_label_dir(
            self.cache_dir,
    ...
    @staticmethod
    def _find_subdir(base: str, candidates: list) -> str:
        """Return the first existing candidate subdirectory, or the first as default."""
    ...
    def _setup_file_paths(self, dataset_dir: str) -> None:
        ...
        for dataset in self.config.DATASET.split('+'):
            dataset_base = os.path.join(dataset_dir, dataset)
            image_dir = self._find_subdir(dataset_base, ['im', 'Image', 'images', 'JPEGImages'])
            label_dir = self._find_subdir(dataset_base, ['gt', 'GT_Object', 'masks', 'SegmentationClass'])
    ...
    def _prepare_cache(self) -> None:
        """Prepare feature cache, writing each feature immediately to avoid RAM accumulation."""
        self.prepare_feature_extractor()
        features_cache = self.cache_manager.get_features_cache()

        for index, img_path in enumerate(track(
            self.image_paths,
            ...
            features_cache.io.write_file(index, features.squeeze(0).to('cpu'))

        # Write index file after all features are saved
        from engine.utils.fileio.backend.ioctl.jsonio import JSONIO
        JSONIO.write_file(features_cache.io.index_path, features_cache.io.index_map)
        features_cache.io.reload_path()
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        pseudo_label = None
        refined_dir = self.refined_pseudo_label_dir
        has_refined_dir = os.path.isdir(refined_dir)
        if has_refined_dir:
            ...
            if os.path.exists(png_path):
                ...
                img = img.resize((feature_size, feature_size), resampling.LANCZOS)
        ...
        if pseudo_label is None:
            ...
            if pseudo_label_cache:
                ...
                # Partial refined-label sets must remain stackable with the
                # resized PNG tensors in the same batch.
                if has_refined_dir and isinstance(pseudo_label, torch.Tensor):
                    feature_size = getattr(self.config, 'feature_size', 68)
                    pseudo_label = pseudo_label.float()
                    if pseudo_label.ndim == 2:
                        pseudo_label = pseudo_label.unsqueeze(0)
                    if pseudo_label.shape[-2:] != (feature_size, feature_size):
                        pseudo_label = torch.nn.functional.interpolate(
                            pseudo_label.unsqueeze(0),
